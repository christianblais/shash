...
describe Shash do
  ...
  it "#initialize should propagates the proc when given" do
    h = Shash.new{ |k| k.to_s }
    h.a = {}
    h.a.b = 1
    h.should == {"a"=>{"b"=>1}}
  end
  ...
  it "should handles all sort of fancy things" do
    h = Shash.new{ |k| k.to_s.upcase }
    h.test = "are you sure?"
    h.has_key?(:test).should == true
    h.has_key?("TEST").should == true
    h.test.should == "are you sure?"
    h.should == {"TEST"=>"are you sure?"}.to_shash
  end
